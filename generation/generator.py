    ...
    return ",\n".join([f"{{\"{name.split(":")[1]}\","
                            f"{{"
                                f"{",\n".join([f"{{nbt::tag_compound{{{",".join([f"{{\"{property}\", \"{property_value}\"}}" for property, property_value in state.state.items()])}}},"
                                                    f"BlockState{{"
                                                        f"\"{name.split(":")[1]}\","
                                                        f"nbt::tag_compound{{{",".join([f"{{\"{property}\", \"{property_value}\"}}" for property, property_value in state.state.items()])}}},"
                                                        f"ColorInfo{{{{{state.color.red},{state.color.green},{state.color.blue},{state.color.alpha}}},"
                                                        f"{state.tint}}}"
                                                    f"}}"
